...
class test_compute_paths(unittest.TestCase):
    ...
    def test_unreported(self):
        ...
        self.assertEquals(len(output_paths[1]), 1)
        # 2 exposed after second contact, so take it as it is closer
        self.assertEquals(output_paths[1][0], (1564450000, 1, 2))

        self.assertEquals(len(output_paths[2]), 2)
        # going through 6 even if it is not reported as it is the only way to reach 7
        self.assertEquals(output_paths[2][0], (1564350000, 1, 6))
    ...
    def test_preferKnownOverUnreported(self):
        ...
        self.assertEquals(len(output_paths[2]), 2)
        # going through 2 instead of 7
        self.assertEquals(output_paths[2][0], (1564450000, 1, 2))
    ...
    def test_before_report(self):
        ...
        self.assertEquals(len(output_paths[0]), 1)
        # self infection path is always at report time if only a single edge is needed. Insunates costs 0, not consistency with other paths
        self.assertEquals(output_paths[0][0], (1564910000, 1, 1))

        self.assertEquals(len(output_paths[1]), 1)
        # it should take the later one of the two (1,2) contacts, as that is closer to the reported time
        self.assertEquals(output_paths[1][0], (1564450000, 1, 2))
    ...
    def test_small(self):
        ...
        self.assertEquals(len(output_paths[0]), 1)
        # self infection path is always at report time if only a single edge is needed.
        self.assertEquals(output_paths[0][0], (1664770000, 8, 8))

        self.assertEquals(len(output_paths[1]), 1)
        # Only path available
        self.assertEquals(output_paths[1][0], (1665770000, 8, 9))
    ...
    def test_WeightDatasetandTreeInDegree(self):
        # known case that went wrong
        filepath_edges = "F:/Development/Swansea/scrc-vis-modelling/ContactTracing/GraphVisualisation/RepresentativeTreesViz/RepresentativeTreeCalculator/reconstructingEpidemic/Data/TestCases/edge15791.tsv"
        filepath_nodes = "F:/Development/Swansea/scrc-vis-modelling/ContactTracing/GraphVisualisation/RepresentativeTreesViz/RepresentativeTreeCalculator/reconstructingEpidemic/Data/TestCases/node15791.tsv"
        K = 1

        output_paths = compute_output_paths(K, filepath_edges, filepath_nodes)

        # Add stuff here that says the output structure is a tree. Possibly can specify it exactly.

        self.assertEqual(len(output_paths), 5)

        # Each node can only be infected by a single node
        infectionMap = dict()
        for path in output_paths:
            for edge in path:
                ...
                if target in infectionMap:  # it in there
                    # must be infected by the same node
                    self.assertEquals(infectionMap[target], source)
                infectionMap[target] = source

        self.assertEqual(len(infectionMap.keys()), 5)
