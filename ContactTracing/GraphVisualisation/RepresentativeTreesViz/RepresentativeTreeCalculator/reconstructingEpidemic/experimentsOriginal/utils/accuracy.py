...
def get_lb_snapshots(sources, immune, reported, snapshots):
    ...
    sorted_infected = sorted([(t, src) for src, t in sources.items() if src in reported['infected']])
    sorted_immune = sorted([(t, node) for node, t in immune.items() if src in reported['recovered']])
    snapshots_time = sorted(snapshots.keys())
    input_infected = set()
    input_immune = set()
    input_snapshots = {}
